...
# coin1_name = 'BTCUSD-1d'
# coin2_name = 'ETHUSD-1d'
coin1_name = 'day-k-沪深300'
coin2_name = 'day-k-创业板指'
commission_rate = 3 / 1000  # 佣金税
stamp_duty_rate = 1 / 1000  # 印花税
...
# 读取数据
coin1_df = pd.read_csv('data/n_days_cmp/%s.csv' % coin1_name, encoding='utf-8', parse_dates=['date'])
coin2_df = pd.read_csv('data/n_days_cmp/%s.csv' % coin2_name, encoding='utf-8', parse_dates=['date'])
...
def strategy1(cpy_coin1, cpy_coin2):
    ...
    # 数字货币从17年开始回测
    df = df[df['date'] >= pd.to_datetime('20170101')]
...
with open('result/n_days_cmp/label-%s-%s.json' % (coin1_name, coin2_name), 'w', encoding='utf-8') as f:
...
plt.savefig('result/n_days_cmp/performance-%s-%s.jpg' % (coin1_name, coin2_name))
# plt.show()
